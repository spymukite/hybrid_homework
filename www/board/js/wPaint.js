...
(function($)
{
	...
	$.fn.wPaint.defaultSettings = {
		...
		strokeStyle			 : '#090738',			// start stroke style
	...
	{	
		...
		{	
			...
			var _self = this;
			var fixwidth=$( window ).width()-60;
            
			$(this.canvas)
			.attr('width', fixwidth + 'px')
		...
		{
			// var text = _self.textInput.val();
			// _self.ctx.fillStyle="black";
			// _self.ctx.fillText(text, _self.textInput.position().left, _self.textInput.position().top);
		...
		drawTextUp: function(e, _self)
		{
			...
			for(var i=0, ii=lines.length; i<ii; i++)
			{
				...
				_self.ctx.fillStyle="black";
				_self.ctx.fillText(lines[i], left + 14, top + 20);
